...
class SafepayGateway extends WC_Payment_Gateway
{
        ...
        add_action('rest_api_init', function () use ($namespace, $permission_callback) {
            $routes = [
                ...
                [
                    'route'   => '/diagnose',
                    'methods' => 'GET',
                    'callback' => [$this, 'safepay_diagnose'],
                ],
            ];
    ...
    /**
     * Diagnostic endpoint — accessible via browser (admin only in production).
     * URL: https://yourstore.com/wp-json/safepay/v1/diagnose?safepay_diag=1
     * REMOVE THIS ROUTE AFTER DEBUGGING.
     */
    public function safepay_diagnose($request)
    {
        // Basic protection: require a query param to prevent casual exposure
        if ($request->get_param('safepay_diag') !== '1') {
            return new WP_Error('forbidden', 'Add ?safepay_diag=1 to the URL.', ['status' => 403]);
        }
        ...
        $report['config'] = [
            'app_env'            => $this->appEnv,
            'base_url'           => $this->get_env_url(),
            'merchant_api_key'   => $this->merchantApiKey ? substr($this->merchantApiKey, 0, 6) . '******' : 'NOT SET',
            'merchant_secret'    => $this->merchantSecret ? 'SET (hidden)' : 'NOT SET',
            'webhook_secret'     => $this->merchantWebhookSecretKey ? 'SET (hidden)' : 'NOT SET',
            'currency'           => get_woocommerce_currency(),
            'is_valid_currency'  => $this->is_valid_for_use() ? 'YES' : 'NO',
            'site_url'           => get_site_url(),
        ];

        // --- 2. Token endpoint connectivity ---
        $baseURL   = $this->get_env_url();
        $tokenUrl  = esc_url_raw($baseURL . SafepayEndpoints::TOKEN_ENDPOINT->value);
        $txUrl     = esc_url_raw($baseURL . SafepayEndpoints::TRANSACTION_ENDPOINT->value);
        ...
        $test_args = [
            ...
            'body' => json_encode([
                ...
                'order_id'         => 0,
                'source'           => 'woocommerce-diag',
            ]),
        ];

        // Token endpoint
        $token_response = wp_remote_post($tokenUrl, $test_args);
        if (is_wp_error($token_response)) {
            $report['token_endpoint'] = [
                'status' => 'WP_ERROR',
                'detail' => $token_response->get_error_message(),
            ];
        } else {
            $report['token_endpoint'] = [
                'status'      => 'REACHABLE',
                'http_code'   => wp_remote_retrieve_response_code($token_response),
                'body_preview'=> substr(wp_remote_retrieve_body($token_response), 0, 300),
            ];
        }

        // Transaction endpoint
        $tx_response = wp_remote_post($txUrl, $test_args);
        if (is_wp_error($tx_response)) {
            $report['transaction_endpoint'] = [
                'status' => 'WP_ERROR',
                'detail' => $tx_response->get_error_message(),
            ];
        } else {
            $report['transaction_endpoint'] = [
                'status'       => 'REACHABLE',
                'http_code'    => wp_remote_retrieve_response_code($tx_response),
                'body_preview' => substr(wp_remote_retrieve_body($tx_response), 0, 300),
            ];
        }

        // --- 3. SSL verification check ---
        $ssl_test = wp_remote_get('https://api.getsafepay.com', ['timeout' => 10, 'sslverify' => true]);
        $report['ssl_verify'] = is_wp_error($ssl_test)
            ? ['status' => 'FAIL', 'detail' => $ssl_test->get_error_message()]
            : ['status' => 'PASS', 'http_code' => wp_remote_retrieve_response_code($ssl_test)];

        // --- 4. PHP + WP environment ---
        $report['environment'] = [
            'php_version'        => PHP_VERSION,
            'wp_version'         => get_bloginfo('version'),
            'wc_version'         => defined('WC_VERSION') ? WC_VERSION : 'unknown',
            'allow_url_fopen'    => ini_get('allow_url_fopen') ? 'ON' : 'OFF',
            'curl_available'     => function_exists('curl_version') ? curl_version()['version'] : 'NOT AVAILABLE',
